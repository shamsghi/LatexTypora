...
- **`\tableofcontents` contents:** `[toc]` prints under a `Contents` heading — or `\contentsname` in the tagged language, from `目录` to `Inhaltsverzeichnis` to `فہرست` — with the section number on every entry, starting at `##` — the `#` title stays out of its own contents, as in LaTeX. Entries are set the way `\@dottedtocline` sets a `.toc` line: every number in a box of its level's width, so titles stand in a column and a title too long for the measure wraps back to that column rather than under its number; `##` entries in bold, opened with `\addvspace` like `\l@section`. The leaders are the one part left out — dots lead the eye to a page number, and a Markdown document has none. Entries are body-black and unruled, the way `hyperref`'s `hidelinks` sets them.
...
| `--toc-number-h2/h3/h4` | `counter(...)` | Entry numbers inside `[toc]`, matching the heading numbers. Set all three to `""`, and the widths below with them, to list the headings unnumbered. |
| `--toc-number-width-h2/h3/h4` | `1.5em`, `2.3em`, `3.2em` | The box each entry number is set in — `\@dottedtocline`'s third argument, which fixes both the column the titles of a level start in and the indent of the level below it. It is where `article.cls` gets the `1.5em` indent of `\subsection` and the `3.8em` of `\subsubsection`. Widen one where a document numbers past what it holds. |
