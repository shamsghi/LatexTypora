...
usage() {
    cat <<EOF
Usage:
  ./install-macos.sh [--theme-dir PATH] [--ref REF]
  curl -fsSL https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/main/install-macos.sh | bash
...
main() {
    ...
    if [[ "${CREATED_THEME_DIR}" -eq 1 ]]; then
        ...
        echo "  ./install-macos.sh --theme-dir \"/path/to/Typora/themes\""
    fi
